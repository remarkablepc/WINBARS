...
[![PowerShell 5.1+](https://img.shields.io/badge/PowerShell-5.1%2B-5391FE?logo=powershell&logoColor=white)](https://microsoft.com)
[![Architecture](https://img.shields.io/badge/Architecture-x64%20%7C%20x86-success)]()
[![Standalone Binary](https://img.shields.io/badge/Binary-WINBARS.exe-informational)]()
[![Zero-Footprint Mode](https://img.shields.io/badge/Zero--Footprint-0%20Resident%20Files-brightgreen)]()
[![License](https://img.shields.io/badge/License-100%25%20Free%20for%20Personal%20%26%20Commercial%20Use-brightgreen)]()
[![Say Thanks](https://img.shields.io/badge/Say%20Thanks-PayPal-00457C?logo=paypal&logoColor=white)](https://www.paypal.com/ncp/payment/EKH76RTYHH24S)
[![GitHub Sponsors](https://img.shields.io/badge/Sponsor-GitHub%20Sponsors-EA4AAA?logo=githubsponsors&logoColor=white)](https://github.com/sponsors/remarkablepc)

---

> [!NOTE]
> ### 📦 Official Standalone Binary Distribution Repository
...
> - **Zero Footprint Audit Guarantee (Mode 0)**: In Zero-Footprint mode, WINBARS leaves **0 resident files and 0 bytes on the host `C:\` drive**. All activity streams live to your console and logs to your portable USB drive (`.\logs\deployment.log`).
...
| **Overall Grade** | **A+ (100 / 100)** | **B+ (86 / 100)** | **A- (90 / 100)** | **A (93 / 100)** | **C+ (68 / 100)** |
...
* **100% Private, Local & Direct Support**: Keeping it closed-source ensures WINBARS stays clean, local, and private, while allowing me to keep the core tool accessible and supported directly by the community without corporate exploitation.
...
Choose the right balance of visibility, permanence, and automation for each workstation:

| Mode & Profile Name | Target Audience & Machine Type | Primary Backup Capabilities (`BACKS UP`) | Resident Footprint & Exclusions (`OMITS`) | Hotkeys & WinPE Recovery Hooks | Tray & ScamBuster Watchdog |
| :--- | :--- | :--- | :--- | :---: | :---: |
| **Mode 0: `ZeroFootprint`** ⭐ | Technicians, MSPs & Sterile Compliance Audits | Daily Restore Point + Robocopy Mirror (30-day recycle bin) + wbadmin Image + BitLocker Keys | **0 MB (0 files on target PC)**; No tray, no resident scripts, no branding | ❌ Zero host files (USB on-demand only) | ❌ None *(Pure Native Windows Engine)* |
| **Mode N: `NearZeroFootprint`** 👻 | Corporate Clients & Anti-Competitor Stealth Shield | Daily Restore Point + Robocopy Mirror to `E:\WindowsBackup\` + wbadmin Image + BitLocker Keys | **0 Resident EXEs/Daemons** (100% native Task Scheduler). Standard 2 unbranded desktop shortcuts + generic Start Menu folder. | ❌ Zero host EXEs (Native `powershell.exe` & `rstrui.exe` shortcuts) | ❌ None *(Pure Native Windows Engine)* |
| **Mode 1: `SystemUndo`** (`Minimal`) | Users wanting rapid OS rollback only (Gamers, Family) | Daily System Restore Point (unthrottled) + VSS Writer Auto-Heal & Shadow Storage Guard | No user data sync, no bare-metal images, no tray, no watchdog | ✅ **WinPE Boot Hooks** • **Panic Hotkey (Ctrl+Win+B)** | ❌ Silent background execution |
| **Mode 2: `LocalDisasterGuard`** | Laptops & single-drive systems without external drive | **[Mode 1 +]** Local partition bare-metal DISM image (`.wim`), driver & MSI install checkpoints | No external drive backup, no user data sync, no tray icon | ✅ **WinPE Boot Hooks** • **Panic Hotkey (Ctrl+Win+B)** | ❌ Silent background execution |
| **Mode 3: `HeadlessFull`** | Desktops & workstations with external drives (silent) | **[Mode 1 +]** Daily differential Robocopy sync, bare-metal images, missing drive connection prompt | No floppy tray icon, no ScamBuster active watchdog. Defaults to Shop Branding (unless `-Vanilla`). | ✅ **WinPE Boot Hooks** • **Panic Hotkey (Ctrl+Win+B)** • **Drive Alerts** | ❌ Silent Task Scheduler operation |
| **Mode 4: `TotalProtection`** (`FullInteractive`) | Complete interactive managed workstation suite (Seniors, VIPs) | **[Mode 3 +]** Complete user data, bare-metal images, restore points, drive alerts | **Zero Omissions** (complete interactive protection suite). Defaults to Shop Branding (unless `-Vanilla`). | ✅ **WinPE Boot Hooks** • **Panic Hotkey (Ctrl+Win+B)** • **Protection Center GUI (Ctrl+Win+W)** | ✅ **Floppy Tray Icon** • **ScamBuster Active Watchdog** |
| **Mode 5+: Custom Profiles** 🛠️ | Custom technician & enterprise deployments | **Dynamically calculated** based on active components configured in `custom_profiles.json` | **Dynamically calculated** based on omitted components | Configurable per-profile | Configurable per-profile |
