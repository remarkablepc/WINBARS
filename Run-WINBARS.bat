...
:: Check config.json for DefaultBrand override (e.g. Vanilla, or specific brand file)
if exist "%~dp0config.json" (
    for /f "tokens=2 delims=:, " %%a in ('findstr /i "\"DefaultBrand\"" "%~dp0config.json" 2^>nul') do (
        set "CFG_DEF_BRAND=%%~a"
...
:: 5. Determine active brand: Match last used brand, then default.json/branding.json, then brand 1
if !BRAND_COUNT! GTR 0 (
    set "BRAND_ARG=-Branded"
    if defined LAST_BRAND_SAVED (
        for /l %%i in (1,1,!BRAND_COUNT!) do (
    ...
    if "!CHOSEN_BRAND_FILE!"=="" (
        set "CHOSEN_BRAND_FILE=!BRAND_1_FILE!"
        set "CHOSEN_BRAND_NAME=!BRAND_1_NAME!"
        set "ACTIVE_BRAND_INDEX=1"
    )
...
:: Dynamically refresh active deployment profile from config.json
set "DETECTED_PROFILE="
if exist "%~dp0config.json" (
    for /f "tokens=2 delims=:, " %%p in ('type "%~dp0config.json" 2^>nul ^| findstr /i "\"DeploymentProfile\""') do set "DETECTED_PROFILE=%%~p"
)
if "!DETECTED_PROFILE!"=="" if exist "C:\ProgramData\WINBARS\config.json" (
    for /f "tokens=2 delims=:, " %%p in ('type "C:\ProgramData\WINBARS\config.json" 2^>nul ^| findstr /i "\"DeploymentProfile\""') do set "DETECTED_PROFILE=%%~p"
)
...
if exist "%~dp0config.json" (
    ...
    for /f "tokens=2 delims=:," %%a in ('type "%~dp0config.json" 2^>nul ^| findstr /i "DefaultMenuChoice"') do (
