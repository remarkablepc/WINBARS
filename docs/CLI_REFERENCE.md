...
| **Mode 1: `SystemUndo`** ⏪ *(Stealth Hardener)* | `Minimal`, `1` | **BACKS UP**: Daily System Restore (unthrottled), VSS Shadow Storage Guard, Driver/MSI install checkpoints, Local Baseline System Image (`C:\SystemRecovery\_baseline.wim` if disk $\ge 25$ GB free).<br>**OMITS**: Zero resident EXEs, no shortcuts, no tray, no watchdog, no third-party branding. 100% native Windows Task Scheduler. | **0 Resident EXEs** *(100% Native Windows Hardening)* | ❌ Zero host hooks *(Native WinRE detects restore points & `.wim` automatically)* | ❌ None *(Pure Native Windows Engine)* |
| **Mode 2: `LocalDisasterGuard`** 💽 | `2` | **BACKS UP**: [Mode 1 +] Local secondary/partition bare-metal DISM image (`.wim`) for offline recovery without an external drive.<br>**OMITS**: No external drive sync, no personal file mirror, no tray, no watchdog. | Local Suite (`~12 MB RAM`) | ✅ **Native WinRE Boot Hook** • **Protection Hotkey (`Ctrl+Win+W`)** • **Panic Hotkey (`Ctrl+Win+B`)** • **Desktop Suite** | ❌ Silent background sentry |
| **Mode 3: `HeadlessFull`** 🏢 | `3` | **BACKS UP**: [Mode 1 +] Daily differential Robocopy sync, scheduled bare-metal images (`.wim` to both local & USB), multi-drive rotation, **missing drive connection prompts**.<br>**OMITS**: No floppy tray icon, no active ScamBuster watchdog, no desktop GUI. Defaults to Shop Branding (unless `-Vanilla`). | Local Suite (`~12 MB RAM`) | ✅ **Native WinRE Boot Hook** • **Protection Hotkey (`Ctrl+Win+W`)** • **Panic Hotkey (`Ctrl+Win+B`)** • **Drive Alerts** | ❌ Silent background sentry |
| **Mode 4: `TotalProtection`** 🛡️ | `FullInteractive`, `4` | **BACKS UP**: [Mode 3 +] Complete personal files, bare-metal images (`.wim` to both local & USB), restore points, drive alerts.<br>**OMITS**: Zero Omissions (complete interactive protection suite). Defaults to Shop Branding (unless `-Vanilla`). | Local Suite (`~16 MB RAM`) | ✅ **Native WinRE Boot Hook** • **Protection Hotkey (`Ctrl+Win+W`)** • **Panic Hotkey (`Ctrl+Win+B`)** • **Protection Center GUI** | ✅ **Floppy Tray Icon** • **ScamBuster Active Watchdog** |
