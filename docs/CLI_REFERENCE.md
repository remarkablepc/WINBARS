...
| **Mode 1: `SystemUndo`** ⏪ *(Stealth Hardener)* | `Minimal`, `1` | **BACKS UP**: Daily System Restore (unthrottled), VSS Shadow Storage Guard, Driver/MSI install checkpoints, Local Baseline System Image (`C:\SystemImages\_baseline.wim` if disk $\ge 25$ GB free).<br>**OMITS**: Zero resident EXEs, no shortcuts, no tray, no watchdog, no third-party branding. 100% native Windows Task Scheduler. | **0 Resident EXEs** *(100% Native Windows Hardening)* | ❌ Zero host hooks *(Native WinRE detects restore points & `.wim` automatically)* | ❌ None *(Pure Native Windows Engine)* |
