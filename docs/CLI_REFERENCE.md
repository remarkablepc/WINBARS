...
| :--- | :--- | :--- | :---: | :---: | :---: |
| **Mode 0: `ZeroFootprint`** ⭐ | `0` | **BACKS UP**: Daily System Restore, User Data (Robocopy Mirror + 30-Day Retention), Bare-Metal Image (wbadmin), BitLocker Keys.<br>**OMITS**: Zero files on PC, no tray, no watchdog, no shortcuts. Pure native Windows engine. | **0 Bytes on PC** *(Runner script & logs strictly on USB)* | ❌ Zero host files (USB on-demand only) | ❌ None *(Pure Native Windows Engine)* |
| **Mode N: `NearZeroFootprint`** 👻 | `NearZero`, `N` | **BACKS UP**: Daily System Restore, User Data (Robocopy Mirror to `E:\WindowsBackup\` + 30-Day Retention), Bare-Metal Image (wbadmin), BitLocker Keys.<br>**OMITS**: Zero resident EXEs/daemons on host. Standard 2 unbranded desktop shortcuts (`Backup Personal Files`, `Windows System Restore`) + generic `System Backup & Recovery` Start Menu folder. Restores via native Explorer & WinRE. Complete stealth anti-competitor defense. | **0 Resident EXEs** *(100% native Task Scheduler)* | ❌ Zero host EXEs (Native `powershell.exe` & `rstrui.exe` shortcuts) | ❌ None *(Pure Native Windows Engine)* |
| **Mode 1: `SystemUndo`** | `Minimal`, `1` | **BACKS UP**: Daily System Restore (unthrottled), VSS Writer Auto-Heal & Shadow Storage Guard, Driver/MSI install checkpoints.<br>**OMITS**: No user data sync, no bare-metal images, no tray, no watchdog. | Local Suite (< 5 MB RAM) | ✅ **WinPE Boot Hooks** • **Panic Hotkey (`Ctrl+Win+B`)** | ❌ Silent background execution |
| **Mode 2: `LocalDisasterGuard`** | `2` | **BACKS UP**: [Mode 1 +] Local secondary/partition bare-metal DISM image (`.wim`) for offline recovery without an external drive.<br>**OMITS**: No external drive sync, no personal file mirror, no tray, no watchdog. | Local Suite | ✅ **WinPE Boot Hooks** • **Panic Hotkey (`Ctrl+Win+B`)** | ❌ Silent background execution |
...
* **Floppy Disk Icon Roles**:
  * 🔵 **Classic Blue Floppy (`app.ico`)**: Protection Center Hub, Suite Manager, System Recovery.
  * 🟣 **Signature Purple Floppy (`app_backup.ico`)**: Backup Action / Data Sync (`Backup Personal Files`, `Fast Backup`).
...
| -DefuseOneDriveNags | Surgically defuses deceptive Windows / OneDrive 'Not Backed Up' scare banners & KFM hijacking. |
...
| :--- | :--- | :--- | :--- | :--- |
| **Mode 4: `TotalProtection`** | Drive Connected | `▶ Backup My Files Now` | `🛡 Backup Files` | `⚡ Backup My Files Now (Files + Checkpoint)` |
