...
* **Problem**: A broken update or conflicting software requires rolling back system state.
...
* **The 2-Step Solution**:
  1. **Step 1 (Base System Image)**: Uses standard Microsoft `dism.exe /Apply-Image` to apply the full bootable operating system partition from `D:\WINBARS_Backup\` back to disk, followed by `bcdboot` EFI bootloader repair.
  2. **Step 2 (Latest User File Sync)**: After DISM finishes, WINBARS automatically detects if more recent user files exist in `D:\WINBARS_Backup\UserBackups\` (from daily/hourly Robocopy mirrors) and prompts the technician/user to restore them, ensuring zero data loss between the base image date and the disaster date.
