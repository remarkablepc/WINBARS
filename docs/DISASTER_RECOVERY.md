...
| **`BitLocker_Recovery_Key.enc` / `.aes`** | Cryptographically secured BitLocker disaster recovery keys (HIPAA § 164.312 compliant) | Encrypted at rest via Shop/Company Master Key (`.enc`) or AES-256 PIN (`.aes`). Accompanied by `BitLocker_Recovery_Key.txt` compliance record. |